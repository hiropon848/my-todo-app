...
## 現在の実装状況
- **実装済み**: 認証システム、ToDo CRUD、プロフィール管理、モーダル統一UI、トースト通知、マスタデータ管理、フィルタリング機能（URLクエリパラメータ連携）、ソート機能、パスワードリセット機能、検索機能（重複ローディング問題解決済み）
...
### 最新の改善履歴
- **検索機能重複ローディング問題**: 完全解決済み、無限ループ解消・executeSearch関数削除・フラグ制御簡素化により根本解決（2025-07-14）
- **フィルタリング機能完全実装**: Phase 1-6完了、URLクエリパラメータ連携、ブラウザ履歴対応、リアルタイム更新（2025-07-08）
- **ソート機能実装**: Phase 8完了、8つのソートオプション、URLパラメータ管理（2025-07-08）
- **検索機能UI実装**: Phase 7実装、バックエンド・フック・UI完了、重複ローディング問題発生（2025-07-08）
...
#### 部分ローディング調査
- **3G回線調査**: 1.2ともに部分ローディングされていない可能性を確認
  - 原因究明のためさらなる調査が必要
  - ネットワーク状況、コンポーネントのレンダリングロジック、データフェッチ方法を詳細に検証する必要がある

---

# ⚠️【最重要・例外なし】LLM実行時強制ルール

## 🔍 実装前チェックリスト（全項目✅必須）
実装開始前に以下を**音読確認**し、**1つでも未完了なら実装禁止**：

- [ ] **ファイル読込**: Read toolで関連ファイルの**実際の内容**を表示済み（Grep検索のみでは不可）
- [ ] **現状確認**: 修正対象のコード・関数・変数が**実際に存在する**ことをRead toolで確認済み
- [ ] **理解度100%**: 動作フローを完全把握（React hooks、非同期処理、状態管理含む）
- [ ] **シミュレーション**: 実装後の動作シミュレーション完了（エッジケース含む）
- [ ] **確実性95%**: 実装可能性確認（推測・希望的観測の完全排除）
- [ ] **不明点ゼロ**: 不明点・推測部分・希望的観測の完全排除
- [ ] **タイミング検証**: タイミング依存処理の検証完了（setTimeout、useEffect等）

**確認方法**: 各項目を声に出して「✅完了」と宣言してから次へ進む

## 🔍 虚偽報告防止ルール（実装前・実装後共通）

### **具体的証拠なしの報告を禁止**
以下の報告は**具体的証拠の提示が必須**：
- 「理解度100%」→ Read tool使用履歴と理解内容の詳細記載必須
- 「確実性95%」→ 技術的根拠と検証内容の明記必須
- 「削除済み」「修正済み」→ Read toolでの確認結果提示必須
- 「品質チェック済み」→ 実際のコマンド実行結果提示必須

### **推測・希望的観測の言い換え禁止**
- 「既に実装済みの可能性」→ **推測表現として即座停止**
- 「おそらく削除されている」→ **推測表現として即座停止**
- 「Step1で解決したはず」→ **推測表現として即座停止**

### **形式的チェックリスト完了の禁止**
- ✅マークは**実際の作業完了後のみ**使用可能
- 「音読確認」は具体的行動を伴う確認のみ有効
- 口頭での「完了宣言」は証拠となる作業履歴が必須

## 🚫 即座停止トリガー（検出したら実装即中止）

以下に該当した瞬間、**即座に実装停止**：
- **推測表現**: 「〜のはず」「〜だと思う」「〜かもしれない」
- **希望的観測**: 「うまくいくはず」「大丈夫だろう」「理論的には正しい」
- **タイミング依存**: 「100ms後なら」「おそらく先に実行される」
- **不確実性認識**: 「根本的な理解が不足している可能性」
- **曖昧な動作保証**: 「動作するはず」「問題ないと思う」

## 📋 品質保証システム（実装毎即座実行）

### 自動承認コマンド（実装後必須実行）
**実装後は即座に以下を実行**（1つでも失敗なら次ステップ進行禁止）：
```bash
npm run lint       # ESLintチェック
npm run typecheck  # TypeScriptコンパイルチェック
...
**品質チェック失敗時は必ず修正完了まで次ステップに進まない**

## 🔒 実装制限事項

### 許可が必要な変更（明示的承認必須）
以下の変更は**必ずユーザーの明示的な許可を得てから実行**：
- 新規ファイルの作成
- 既存ファイルの変更
- データベース構造の変更
- 依存関係（パッケージ）の追加・更新
...
### 禁止事項（**絶対違反禁止**）
- mdファイルに記載されていないstep名、項目名、項目番号をターミナル上で表示すること
- 許可なく修正すること
- 指示者の質問中に許可なく実装すること
- 指示者の質問中に許可なく修正すること
- 指示者の確認中に許可なく実装すること
- 指示者の確認中に許可なく修正すること
- Read tool使用なしで「ファイル読込完了」「理解度100%」を報告すること
- 推測・仮定に基づいて「確実性95%」「削除済み」「修正済み」を報告すること
- Grep検索結果のみでファイルの全内容を理解したと報告すること
- 実装対象コードの存在確認なしに作業を開始すること

### 必須事項（省略不可）
- 計画前に関連する全ファイルを読み込み、動作フローを完全に理解したうえで計画、提案すること
- 実装前に関連する全ファイルを読み込み、動作フローを完全に理解すること
- 実装中の機能に関連するソースコードを理解した上で原因究明すること
- 実装後は必ず自己検証を行い、期待通りの動作をシミュレートすること
- **品質チェック3点セット**を実装毎に即座に実行すること
- **品質チェック失敗時は必ず修正完了まで次ステップに進まない**こと
- **実装完了報告時は品質チェック結果を必ず記載**すること
...
```
## ✅ 実装完了報告
**理解度**: 100%（動作フローを完全把握、推測ゼロ）
**確実性**: 95%以上（不明点・推測部分なし）
**品質チェック**: 
- ESLint: ✅/❌ [エラー詳細]
- TypeScript: ✅/❌ [エラー詳細]
- Build: ✅/❌ [エラー詳細]
**動作保証**: シミュレーション済み（エッジケース含む）
**実装根拠**: [技術的根拠を明確に記載]
```
...
4. **実装後の確認**:
   - 承認された内容との一致を確認
   - **品質チェック3点セット実行**: `npm run lint` → `npm run typecheck` → `npm run build`
   - **品質チェック結果の報告**: 各コマンドの実行結果を明記
   - **必須フォーマットでの完了報告**

---

**🔥 最終確認リマインダー 🔥**
推測ゼロ・許可確認・品質チェック・虚偽報告防止・Read tool実使用確認 - この5つを忘れたら即座停止

**作業前必須確認**:
「Read toolで実際のファイル内容を確認したか？」「推測・仮定で報告していないか？」「実装対象コードの存在を確認したか？」