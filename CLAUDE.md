...
- **必須事項**
  - 計画前にに関連する全ファイルを読み込み、動作フローを完全に理解したうえで計画、提案すること。
  - 実装前に関連する全ファイルを読み込み、動作フローを完全に理解すること。
  - 実装中の機能に関連するソースコードを理解した上で原因究明すること。
  - 実装後は必ず自己検証を行い、期待通りの動作をシミュレートすること。
  - **品質チェック3点セットを実装毎に即座に実行すること**: `npm run lint` → `npm run typecheck` → `npm run build`
  - **品質チェック失敗時は必ず修正完了まで次ステップに進まないこと**
  - **実装完了報告時は品質チェック結果を必ず記載すること**

### 実装前チェックリスト（必須・例外なし）
実装開始前に以下の全項目を確認し、**1つでも未完了なら実装禁止**：
```
□ 関連する全ファイルの読み込み完了（推測・省略一切なし）
□ 動作フローの完全理解完了（React hooks、非同期処理、状態管理含む）
□ 実装後の動作シミュレーション完了（エッジケース含む）
□ 確実性95%以上での実装可能性確認
□ 不明点・推測部分・希望的観測の完全排除
□ タイミング依存処理の検証完了（setTimeout、useEffect等）
```

### 実装中の強制停止条件
以下に該当した瞬間、**即座に実装停止**：
- **推測表現**: 「〜のはず」「〜だと思う」「〜かもしれない」
- **希望的観測**: 「うまくいくはず」「大丈夫だろう」
- **タイミング依存**: 「100ms後なら」「おそらく先に実行される」
- **不確実性の認識**: 「根本的な理解が不足している可能性」
- **曖昧な動作保証**: 「理論的には正しい」「動作するはず」
